...
namespace Unity.InterceptionExtension.Serialization.Serializable
{
    ...
    public class SerializableDefaultInterceptor : DefaultInterceptor
    {
        ...
        public override void AddPolicies(Type serviceType, Type implementationType, string name, IPolicyList policies)
        {
            ...
            dynamic policy = policies.Get<ITypeInterceptionPolicy>(implementationType);
            if (policy != null)
            {
                policies.Set<ITypeInterceptionPolicy>(new SerializableInterceptorPolicy(policy), implementationType);
            }

            policy = policies.Get<IInstanceInterceptionPolicy>(implementationType);
            if (policy != null)
            {
                policies.Set<IInstanceInterceptionPolicy>(new SerializableInterceptorPolicy(policy), implementationType);
            }
        }
    }
